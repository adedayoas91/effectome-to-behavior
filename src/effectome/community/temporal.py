...
def _viterbi_assign(unary: np.ndarray, penalty: float) -> np.ndarray:
...
def _objective(features: np.ndarray, labels: np.ndarray, centroids: np.ndarray, penalty: float) -> float:
    total = 0.0
    for neuron in range(features.shape[1]):
        ...
        total += float(penalty * np.sum(labels[1:, neuron] != labels[:-1, neuron]))
    return total
...
def _flexibility(labels: np.ndarray) -> np.ndarray:
    if labels.shape[0] <= 1:
        return np.zeros(labels.shape[1], dtype=np.float64)
    return np.mean(labels[1:] != labels[:-1], axis=0)
...
class _TemporalRegularizedMixin:
    ...
    def _fit_retro_run(self, features: np.ndarray, seed: int) -> tuple[np.ndarray, np.ndarray, float]:
        settings = self._settings()
        flat = features.reshape(-1, features.shape[-1])
        ...
        for _ in range(settings.max_iter):
            ...
            for neuron in range(features.shape[1]):
                unary = _sqdist(features[:, neuron, :], centroids)
                updated[:, neuron] = _viterbi_assign(unary, settings.temporal_penalty)
            ...
            if np.array_equal(updated, labels) and np.allclose(new_centroids, centroids):
                ...
                break
            labels, centroids = updated, new_centroids
        return labels, centroids, _objective(features, labels, centroids, settings.temporal_penalty)

    def _fit_prospective_run(self, features: np.ndarray, seed: int) -> tuple[np.ndarray, np.ndarray, float]:
        settings = self._settings()
    ...
    def _fit_runs(self, features: np.ndarray) -> tuple[list[np.ndarray], list[float]]:
        settings = self._settings()
        ...
        for run_idx in range(settings.n_runs):
            ...
            if settings.mode == "prospective":
                labels, _, score = self._fit_prospective_run(features, seed)
            elif settings.mode == "retrospective":
                labels, _, score = self._fit_retro_run(features, seed)
            else:
    ...
    def _package(self, runs: list[np.ndarray], scores: list[float]) -> CommunitySeries:
        settings = self._settings()
        ...
        result = CommunitySeries(labels=consensus, method=self.cfg.name, n_communities_per_window=counts)
        switching = consensus[1:] != consensus[:-1]
        result.__dict__.update(
            {
                "mode": settings.mode,
                "flexibility": _flexibility(consensus),
                "switching": switching,
                "switching_rate": float(np.mean(switching)) if switching.size else 0.0,
...
class TemporalCommunity(_TemporalRegularizedMixin, CommunityDetector):
    ...
    def run(self, series: ConnectivitySeries) -> CommunitySeries:
        ...
        runs, scores = self._fit_runs(features)
        return self._package(runs, scores)
...
class TemporalGreedyCommunity(CommunityDetector):
    ...
    def run(self, series: ConnectivitySeries) -> CommunitySeries:
        ...
        return CommunitySeries(labels=labels, method=self.cfg.name, n_communities_per_window=counts)
